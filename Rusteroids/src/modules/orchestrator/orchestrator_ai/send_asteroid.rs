        ...
        PlanetToOrchestrator::AsteroidAck { planet_id, rocket } => {
            match rocket {
                Some(_rocket) => {
                    ...
                    payload.insert(
                        "message".into(),
                        format!("Asteroids destroyed by Planet #{}.", planet_id),
                    );
                ...
                None => {
                    ...
                    payload.insert(
                        "message".into(),
                        format!("AsteroidAck from Planet #{} !! NO ROCKET !!.", planet_id),
                    );
                    ...
                    if let Some(exp_id) = explorer_to_kill {
                        if let Some((tx_orch_to_exp, _, _, _)) = orch.explorer_channels.get(&exp_id)
                        {
                            ...
                            orch.add_log(format!(
                                "Planet #{} destroyed: Explorer #{} is dead.",
                                target, exp_id
                            ));
                            ...
                            payload.insert(
                                "message".into(),
                                format!("Explorer #{} died on planet #{}.", exp_id, target),
                            );
                    ...
                    match kill_ack {
                        PlanetToOrchestrator::KillPlanetResult { planet_id } => {
                            ...
                            payload.insert(
                                "message".into(),
                                format!("Planet #{} is destroyed.", planet_id),
                            );
