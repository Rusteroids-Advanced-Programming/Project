...
fn send_message_to_planet_with_timeout(
...
pub trait ExplorerAI {
    fn start_ai<F>(&self, custom_handler: F)
    where
        F: Fn();
    fn reset_ai<F>(&self, custom_handler: F)
    where
        F: Fn();
    fn kill<F>(&self, custom_handler: F)
    where
        F: Fn();
    fn move_to_planet<F>(
        &self,
    ...
    fn generate_resource<F>(
        &self,
        to_generate: BasicResourceType,
        custom_handler: F,
    ) -> Result<(), String>
    where
        F: Fn(&Option<&BasicResource>);
    fn combine_resource<F>(&self, to_generate: ComplexResourceType, custom_handler: F)
    where
        F: Fn(&Result<&ComplexResource, &(String, GenericResource, GenericResource)>);
    fn get_bag(&self);
...
impl ExplorerAI for ExplorerBase {
    ...
    fn ask_supported_resources(&self) {
        ...
        if let Some(to_planet) = to_planet_guard.as_ref()
            && let Some(from_planet) = from_planet_guard.as_ref()
        {
            let resp = send_message_to_planet_with_timeout(
                to_planet,
                from_planet,
            ...
            && let Some(from_planet) = from_planet_guard.as_ref()
        {
            ...
            match resp {
                Some(msg) => match msg {
                    ...
                    msg => {
                        println!(
                            "Received unexpected message: {:?} while waiting for SupportedCombinationResponse",
                            msg
                        );
                    }
                },
            ...
            match resp {
                Some(msg) => {
                    ...
                    match msg {
                        ...
                        msg => {
                            println!(
                                "Received unexpected message: {:?} while waiting for GenerateResourceResponse",
                                msg
                            );
                ...
                None => Err(format!(
                    "Failed to generate resource {:?}, planet exceeded timeout",
                    to_generate
                )),
            }
        } else {
            Err(format!(
                "Failed to generate resource {:?}, planet channels are dead",
                to_generate
            ))
        }
    }
                ...
                match resp {
                    Some(msg) => {
                        ...
                        match msg {
                            ...
                            msg => {
                                println!(
                                    "Received unexpected message: {:?} while waiting for CombineResourceResponse",
                                    msg
                                );
                                result = Err("Couldn't combine resource".to_string());
                            }
            ...
            && let Some(from_planet) = from_planet_guard.as_ref()
        {
            ...
            match resp {
                Some(msg) => match msg {
                    ...
                    msg => {
                        println!(
                            "Unexpected message received while waiting for AvailableEnergyCellResponse: {:?}",
                            msg
                        );
                    }
                },
